...
def compute_lat_lng(lat_deg, lng_deg, bearing_deg, dist_mi):
	...
	realtive_dist =  dist_mi * KM_PER_MILE / EARTH_RADIUS_KM
    lat2 = math.asin(math.sin(lat1) * math.cos(relative_dist) + 
                     math.cos(lat1) * math.sin(relative_dist) * math.cos(bearing))
    lng2 = lng1 + math.atan2(math.sin(bearing) * 
                             math.sin(relative_dist) * math.cos(lat1),
                             math.cos(relative_dist) - math.sin(lat1) * math.sin(lat2))
	return (lat2, lng2)
