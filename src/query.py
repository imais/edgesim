	...
	def create_queries(self, levels):
		...
		for i in range(test_queries):
			...
			try:
				...
				elif 0 < len(city_others_ids) and dest_type == QueryDestType.CITY_OTHER:
					dest_id = random.choice(city_others_ids)
				elif dest_type == QueryDestType.COUNTY_SELF:
					dest_id = county.name
				elif 0 < len(county_others_ids) and dest_type == QueryDestType.COUNTY_OTHER:
					dest_id = random.choice(county_others_ids)
				elif dest_type == QueryDestType.STATE_SELF:
					dest_id = state.name
				elif 0 < len(state_others_ids) and dest_type == QueryDestType.STATE_OTHER:
					dest_id = random.choice(state_others_ids)
				else:
